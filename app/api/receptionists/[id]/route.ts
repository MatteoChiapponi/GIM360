...
import { updateReceptionist, deleteReceptionist } from "@/modules/receptionists/receptionists.service"
import { updateReceptionistSchema } from "@/modules/receptionists/receptionists.schema"
...
export const PATCH = withAuthParams<Params>([UserRole.OWNER], async (req, session, { id }) => {
  ...
  const parsed = updateReceptionistSchema.safeParse(body)
  if (!parsed.success) {
    logger.warn("Validation error", { errors: parsed.error.flatten() })
    return NextResponse.json({ error: parsed.error.flatten() }, { status: 400 })
  }

  const result = await updateReceptionist(id, parsed.data)
  logger.info("Receptionist updated", { id })
...
export const DELETE = withAuthParams<Params>([UserRole.OWNER], async (req, session, { id }) => {
  ...
  try {
    ...
    logger.info("Receptionist deleted", { id })
    ...
    logger.error("deleteReceptionist failed", { error: String(err), receptionistId: id })
    throw err
  }
})
